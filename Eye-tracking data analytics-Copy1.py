...
df = pd.read_csv('/Users/maxingjian/Desktop/Eye tracking data analysis/combined data/IDBY01.csv')
alldf = df[(df.Subject == 'IDBY01') & (df.Drive == 'Drive1')]
...
count = 0 # number of pattern sets
t_matrices = {}
...
for i in range(k):
    ...
    # if mask_num of a set of patterns < 5, set this "most unique" pattern to NA, discard this set and move to loop
    if mask_num < 5:
    ...
    for t in tempdf['rec_time_s'].iloc[np.where(mask==count)[0]]:
        ...
        t_matrix = np.append(t_matrix, t)
    t_matrices[i] = t_matrix
...
# we want k sets of pattern as final result
if count < k:
...
for i in range(k):
    for t in range(snippets_aoi.shape[1]):
...
sns.set(rc={'figure.figsize':(30,18)}) # from white to black
fig, axs = plt.subplots(3,4)
for i in range(k):
    for t in range(snippets_aoi.shape[1]):
        # 计算灰度值
        gray = t / (snippets_aoi.shape[1] - 1)
        color = cm.gray(gray)  # 根据灰度值生成颜色
        axs.flat[i].plot(snippets_x[i,t],-snippets_y[i,t], 'o', color=color, markersize=10)#sns.color_palette("tab20")[i])
    axs.flat[i].set_xlim([0,1])
    axs.flat[i].set_ylim([-1,0])
plt.show()
...
sns.set(rc={'figure.figsize':(30,18)})
fig, axs = plt.subplots(3,4)
for i in range(len(snippet_xavg)):
    for t in range(snippet_xavg[i].shape[0]):
        gray = t / (snippet_xavg[i].shape[0] - 1)
        color = cm.gray(gray)
        axs.flat[i].plot(snippet_xavg[i][t],-snippet_yavg[i][t], 'o', color=color, markersize=10)#sns.color_palette("tab20")[i])
    axs.flat[i].set_xlim([0,1])
    axs.flat[i].set_ylim([-1,0])
plt.show()
